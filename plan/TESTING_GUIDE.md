...
test.describe('Setlist Management', () => {
  ...
  test('reorders songs in setlist', async ({ page }) => {
    ...
    // Verify reordering
    await expect(page.locator('[data-testid="setlist-song"]').first()).not.toContainText('Amazing Grace');
  });
});
```
...
describe('SongsAPI', () => {
  ...
  describe('getAvailableSongs', () => {
    ...
    it('excludes recently used songs', async () => {
      ...
      mockPb.collection().getFullList
        .mockResolvedValueOnce(recentUsage)

...
class AuthStore {
  ...
  async switchOrganization(organizationId: string): Promise<void> {
    try {
      ...
      const membership = await pb.collection('organization_memberships').getFirstListItem(
  ...
  canManageUsers(): boolean {
    return this.hasPermission('users:manage') || this.membership?.role === 'admin' || this.membership?.role === 'owner';
  }
...
export class OrganizationsAPI {
  ...
  static async acceptInvitation(token: string): Promise<Organization> {
    const invitation = await pb.collection('organization_invitations').getFirstListItem(
   ...
   */
  static async updateMember(membershipId: string, data: Partial<OrganizationMembership>): Promise<OrganizationMembership> {
...
<div class="relative">
  ...
  {#if showDropdown}
    <div class="absolute top-full left-0 mt-1 w-64 bg-white border border-gray-300 rounded-md shadow-lg z-50">
      <div class="py-1">
        {#each auth.availableOrganizations as org}
          <button
            onclick={() => handleSwitchOrganization(org.id)}
            class="w-full px-4 py-2 text-left text-sm hover:bg-gray-100 {auth.currentOrganization?.id === org.id ? 'bg-blue-50 text-blue-700' : 'text-gray-700'}"
          >
        ...
        <a
          ...
          class="flex items-center space-x-2 w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100"
        >
          <Plus class="w-4 h-4" />
    ...
    <div class="lg:col-span-2">
      <Card>
        <div class="space-y-6">
          ...
          <form onsubmit={(e) => { e.preventDefault(); handleSaveSettings(); }} class="space-y-4">
            <Input
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-2 text-sm">
            <div class="flex justify-between">
              ...
              <span class="font-medium capitalize">{auth.currentOrganization?.subscription_type}</span>
            </div>
            <div class="flex justify-between">
          ...
          <div class="pt-4 border-t">
            <a href="/organization/members" class="text-blue-600 hover:text-blue-800 text-sm">
              Manage Members →
            </a>
          </div>
        </div>

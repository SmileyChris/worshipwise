...
<script lang="ts">
	...
	let { data, onUpdate = () => {}, class: className = '', ...restProps }: Props = $props();
...
<script lang="ts">
	...
	let totalDuration = $derived(songs.reduce((sum, song) => sum + (song.duration_seconds || 0), 0));
...
<script lang="ts">
	...
	// Props destructuring
	let { song, editable = false, onUpdate = () => {}, class: className = '' }: Props = $props();
...
<script lang="ts">
	...
	let { open = false, title = '', size = 'md', children, footer }: Props = $props();
...
{#if open}
	...
	>
		<!-- Backdrop -->
		<div class="bg-opacity-50 fixed inset-0 bg-black" onclick={handleClose}></div>

		<!-- Modal -->
		<div
			class="relative rounded-lg bg-white shadow-xl {sizeClasses} mx-4 w-full"
...
<script lang="ts">
	...
	let usageStatus = $derived(showUsageIndicator ? getSongUsageStatus(song) : null);
...
<div class="rounded-lg border bg-white p-4 shadow-sm transition-shadow hover:shadow-md {className}">
	<div class="flex items-start justify-between">
		<div class="flex-1">
			...
			<div class="mt-2 flex items-center gap-2">
				...
				{#if usageStatus}
					<span class="rounded px-2 py-1 text-xs font-medium {statusColors}">
						{usageStatus === 'red'
							? 'Recently Used'
							: usageStatus === 'yellow'
								? 'Used Recently'
...
<div class="relative">
	...
	{#if isExpanded}
		<div
			id={menuId}
			class="absolute top-full left-0 mt-1 rounded-md border bg-white shadow-lg"
			role="menu"

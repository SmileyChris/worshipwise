...
migrate((app) => {
  // Drop collection (view) if exists
  try {
  ...
  const collection = new Collection({
    ...
    viewQuery: `
      SELECT
        ...
        (SELECT MAX(used_date) FROM song_usage WHERE song_id = s.id AND julianday(used_date) <= julianday('now')) as last_sung_date,
  ...
  // Down migration - revert to simplified string comparison (or previous state)
  try {
    const old = app.findCollectionByNameOrId('songs_enriched');
    if (old) {
      app.delete(old);

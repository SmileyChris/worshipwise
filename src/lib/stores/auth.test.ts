...
describe('AuthStore', () => {
  ...
  describe('Profile Updates', () => {
    ...
    it('should update profile information successfully', async () => {
      const mockUser = { id: 'user1', email: 'test@example.com' } as User;
      const mockProfile = { id: 'profile1', user_id: 'user1', name: 'Old Name', role: 'musician' } as Profile;
  ...
  describe('Password Reset', () => {
    ...
    it('should confirm password reset successfully', async () => {
      ...
      expect(usersCollection.confirmPasswordReset).toHaveBeenCalledWith(token, password, passwordConfirm);
      expect(auth.loading).toBe(false);
      expect(auth.error).toBeNull();
    });

...
export type Permission = 'manage-songs' | 'manage-services' | 'manage-members' | 'manage-church';
...
export function hasPermission(userRoles: UserRole[], permission: Permission): boolean {
	return userRoles.some((userRole) => userRole.expand?.role_id?.permissions?.includes(permission));
}
...
export function hasSkill(userSkills: UserSkill[], skillSlug: string): boolean {
	return userSkills.some((userSkill) => userSkill.expand?.skill_id?.slug === skillSlug);
}

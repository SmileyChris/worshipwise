 ...
 * - upcoming: Scheduled for a future service (blue)
 ...
 */
export type SongUsageStatus = 'upcoming' | 'available' | 'caution' | 'recent' | 'stale';
...
export function isSongUsageStatus(value: string): value is SongUsageStatus {
	return ['upcoming', 'available', 'caution', 'recent', 'stale'].includes(value);
}

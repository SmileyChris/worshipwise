 ...
 */
export function findOptimalInsertionPosition(songs: ServiceSong[], sectionType: string): number {
	...
	for (let i = 0; i < songs.length; i++) {
		const currentSectionOrder = SECTION_TYPES.indexOf(
			songs[i].section_type as (typeof SECTION_TYPES)[number]
		);
...
export function validateServiceFlow(songs: ServiceSong[]): {
	...
	} else if (totalDuration < 900 && songs.length > 2) {
		// 15 minutes

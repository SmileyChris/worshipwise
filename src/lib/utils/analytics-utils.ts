...
export function aggregateUsageData(
  ...
  // Find most used song
  const songUsageCounts = usageData.reduce((acc, usage) => {
    const key = `${usage.song_id}:${usage.title}`;
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);
  
  const mostUsedEntry = Object.entries(songUsageCounts)
    .sort(([, a], [, b]) => b - a)[0];
  ...
  const serviceTypeCounts = setlists.reduce((acc, setlist) => {
...
export function calculateTrends(
  ...
  // Group by date
  const dailyCounts = filteredData.reduce((acc, usage) => {
...
export function generateInsights(
  ...
  if (overview.mostUsedSong.count > 5) {
    insights.push(`"${overview.mostUsedSong.title}" is your most popular song with ${overview.mostUsedSong.count} uses`);
  }
 ...
 */
export function calculateKeyUsageStats(usageData: UsageData[]): Array<{ key: string; count: number; percentage: number }> {
  const keyCounts = usageData.reduce((acc, usage) => {

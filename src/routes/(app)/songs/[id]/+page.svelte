		...
		switch (status) {
			case 'recent':
				return {
					...
					text: daysSince !== undefined && daysSince < Infinity 
...
<div class="space-y-6">
	...
	<div class="md:flex md:items-start md:justify-between">
		<div class="min-w-0 flex-1">
			<h1 class="text-3xl font-bold text-gray-900 sm:text-4xl font-title">
				{song.title}
			</h1>
			{#if song.artist}
		...
		<div class="mt-4 flex flex-col gap-2 md:mt-0 md:ml-6">
			{#if auth.canManageSongs}
				<Button variant="primary" href="/songs?edit={song.id}">
					Edit Song
				</Button>
			{/if}
	...
	<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
		...
		<div class="lg:col-span-2 space-y-6">
			<!-- Song Details -->
			<Card>
				<h2 class="text-lg font-semibold text-gray-900 mb-4 font-title">Song Details</h2>
				
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					{#if song.key_signature}
						<div>
							<dt class="text-sm font-medium text-gray-500">Key</dt>
							<dd class="text-sm text-gray-900 font-medium">{song.key_signature}</dd>
						</div>
					{/if}
			...
			{#if song.lyrics}
				<Card>
					<h2 class="text-lg font-semibold text-gray-900 mb-4 font-title">Lyrics</h2>
			...
			{#if song.chord_chart || song.audio_file || (song.sheet_music && song.sheet_music.length > 0)}
				<Card>
					<h2 class="text-lg font-semibold text-gray-900 mb-4 font-title">Files</h2>
					<div class="space-y-3">
						{#if song.chord_chart}
							<div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
								...
								<Button size="sm" variant="ghost" href={getFileUrl(song.chord_chart)} target="_blank">
									View
								</Button>
							</div>
						{/if}
						...
						{#if song.audio_file}
							<div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
								...
								<Button size="sm" variant="ghost" href={getFileUrl(song.audio_file)} target="_blank">
									Play
								</Button>
							</div>
						{/if}
			...
			{#if song.copyright_info || song.notes}
				<Card>
					<h2 class="text-lg font-semibold text-gray-900 mb-4 font-title">Additional Information</h2>
					
					{#if song.copyright_info}
						<div class="mb-4">
							<dt class="text-sm font-medium text-gray-500 mb-1">Copyright</dt>
					...
					{#if song.notes}
						<div>
							...
							<dd class="text-sm text-gray-700 whitespace-pre-wrap">{song.notes}</dd>
						</div>
					{/if}
				</Card>
			{/if}
		...
		<div class="space-y-6">
			<!-- Usage Statistics -->
			<Card>
				<h2 class="text-lg font-semibold text-gray-900 mb-4 font-title">Usage Statistics</h2>
				<div class="space-y-4">
					<div class="text-center">
						<div class="text-2xl font-bold text-primary font-title">{usageStats.totalUses}</div>
			...
			{#if usageHistory.length > 0}
				<Card>
					<h2 class="text-lg font-semibold text-gray-900 mb-4 font-title">Recent Usage</h2>
					<div class="space-y-3">
							...
							<div class="flex justify-between items-start text-sm">
								<div>
									...
									{#if usage.service_type}
										<div class="text-gray-500 text-xs">{usage.service_type}</div>
									{/if}
								</div>
			...
			{:else}
				<Card>
					<h2 class="text-lg font-semibold text-gray-900 mb-4 font-title">Usage History</h2>

 ...
 */
export function mockAuthenticatedUser(options: {
  userId?: string;
  email?: string;
  name?: string;
  profileName?: string;
 ...
 */
export function mockSuccessfulRegistration(userData?: Partial<User>, profileData?: Partial<Profile>) {
  const mockUser = {
    id: 'user1',
 ...
 */
export function mockSuccessfulProfileUpdate(userUpdates?: Partial<User>, profileUpdates?: Partial<Profile>) {
  const usersCollection = mockPb.collection('users');
  const profilesCollection = mockPb.collection('profiles');
 ...
 */
export function mockFailedPasswordChange(reason: 'invalid_current' | 'update_failed' = 'invalid_current') {
  const usersCollection = mockPb.collection('users');
 ...
 */
export function mockFailedPasswordReset(operation: 'request' | 'confirm', errorMessage = 'Reset failed') {
  const mockError = {
    response: {
      data: { message: errorMessage }
    }
 ...
 */
export function createTestUser(role: 'musician' | 'leader' | 'admin' = 'musician', overrides: {
  user?: Partial<User>;
